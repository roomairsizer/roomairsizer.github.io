  ...
  function init() {
    ...
    function updateRecommendations(requiredCadr, concern) {
      ...
      grid.innerHTML = top3.map((product, idx) => {
        ...
        const link = `https://www.amazon.com/dp/${product.asin}/?tag=${AMAZON_TAG}`;
        
        return `
          <article class="pick">
            <p class="pick__tag">${tag}</p>
            <h4 class="pick__name">${product.name}</h4>
            <p class="pick__meta" style="font-size: 0.875rem; color: var(--text-muted); margin-top: -0.25rem; margin-bottom: 0.75rem;">CADR: ${product.cadr} &nbsp;|&nbsp; Price: ${product.priceTier}</p>
